...
/*
* This code is based of the code from OneLoneCoder:
	...
	while (!bGameOver)
	{
		...
		if (bForceDown)
		{
			...
			else if(!DoesPieceFit(nCurrentPiece, nCurrentRotation, nCurrentX, nCurrentY + 1))
			{
				...
				// Reset pieces to their original state after a piece is locked down
				tetromino[0] = (L"..X...X...X...X.");
				tetromino[1] = (L"..X..XX..X......");
				tetromino[2] = (L".X...XX...X.....");
				tetromino[3] = (L".....XX..XX.....");
		...
		for (auto& object : listObjects)
		{
			...
			*/
			...
			// If it doesn't collide it draws the bullet
		...
		for (int px = 0; px < 4; px++)
		{
			for (int py = 0; py < 4; py++)
			{
				// Check if there is a bullet before drawing.
				if (screen[(nCurrentY + py + 4) * nScreenWidth + (nCurrentX + px + nFieldOffset)] == 'O')
				{
					// If there is a bullet in that space, it replaces that part with a '.'
					tetromino[nCurrentPiece][Rotate(px, py, nCurrentRotation)] = L'.';
				}

				// Draw a '+' on the screen if there should be a piece on that position.
				if (tetromino[nCurrentPiece][Rotate(px, py, nCurrentRotation)] != L'.')
